...
from typing import Callable, List, Tuple, Union, Type, Optional, Sequence
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.attn(x)
        # x = self.Space_Adapter(x) # Adapter
...
class CellViTSAM(nn.Module):
    ...
    def __init__(
        ...
        self.patch_size = 16
        self.input_channels = input_classes # RGB
        self.mlp_ratio = 4
        self.qkv_bias = True
        ...
        self.decoder_t = CellViT(num_classes=oputput_num_classes,embed_dim=self.embed_dim,input_channels=self.input_channels,drop_rate=0.1)
...
class SAM_Model(nn.Module):
    def __init__(
        ...
        self.branch = CellViTSAM(model_path=self.model_path,num_cell_classes=3,num_tissue_classes=2,vit_structure='SAM-H',freeze_encoder=True)
    ...
    def forward(self, id, tissue, cell, mask_meta):
        B,C,H,W = tissue.shape # [2 3 1024 1024]
        ...
        for i in range(len(id)):
            ...
            ## stop grad
            cropped_tissue = cropped_tissue.detach()
            cropped_tissue0 = self.cell_conv0(cropped_tissue.unsqueeze(0))
            cropped_tissue1 = self.cell_conv1(cropped_tissue.unsqueeze(0))
            
            resized_pred0 = F1.resize(cropped_tissue0, re_size)
            resized_pred1 = F1.resize(cropped_tissue1, re_size1)
            res0.append(resized_pred0)
            res1.append(resized_pred1)
        expand_tensor0 = torch.cat(res0, dim=0)
        expand_tensor1 = torch.cat(res1, dim=0)
        pred_cell = self.branch(cell,[expand_tensor0,expand_tensor1],'C')
